...
function Gatherer_Round(x)
  if (x - math.floor(x) > 0.5) then x = x + 0.5; end
  return math.floor(x);
end
  ...
  if (inputName) then
    ...
    if (found > 0) then
      ...
      if (GetLocale() == "ruRU") then
        ...
        name = string.upper(string.sub(inputName, 1, 1)) ..
                   (string.sub(inputName, 2) or "");
      end
    end
...
function Gatherer_OnLoad()
  ...
  SlashCmdList["GATHER"] = function(msg) Gatherer_Command(msg); end
end
...
function Gatherer_Command(command)
  ...
  if ((cmd == "") or (cmd == "help")) then
    ...
    Gatherer_ChatPrint("  |cffffffff/gather (on|off|toggle)|r |cff2040ff[" ..
                           useMinimap ..
                           "]|r - turns the gather minimap display on and off");
    Gatherer_ChatPrint(
        "  |cffffffff/gather mainmap (on|off|toggle)|r |cff2040ff[" ..
            useMainmap .. "]|r - turns the gather mainmap display on and off");
    Gatherer_ChatPrint(
        "  |cffffffff/gather minder (on|off|toggle|<n>)|r |cff2040ff[" ..
            mapMinder ..
            "]|r - turns the gather map minder on and off (remembers and reopens your last open main map; within " ..
            minderTime .. ")");
    Gatherer_ChatPrint("  |cffffffff/gather dist <n>|r |cff2040ff[" ..
                           SETTINGS.maxDist ..
                           "]|r - sets the maximum search distance for display (0=infinite(default), typical=10)");
    Gatherer_ChatPrint("  |cffffffff/gather num <n>|r |cff2040ff[" ..
                           SETTINGS.number ..
                           "]|r - sets the maximum number of items to display (default=10, up to 25)");
    Gatherer_ChatPrint("  |cffffffff/gather fdist <n>|r |cff2040ff[" ..
                           SETTINGS.fadeDist ..
                           "]|r - sets a fade distance (in units) for the icons to fade out by (default = 20)");
    Gatherer_ChatPrint("  |cffffffff/gather fperc <n>|r |cff2040ff[" ..
                           SETTINGS.fadePerc ..
                           "]|r - sets the percentage for fade at max fade distance (default = 80 [=80% faded])");
    Gatherer_ChatPrint("  |cffffffff/gather theme <name>|r |cff2040ff[" ..
                           SETTINGS.iconSet ..
                           "]|r - sets the icon theme: original, shaded (default), iconic or iconshade");
    Gatherer_ChatPrint("  |cffffffff/gather idist <n>|r |cff2040ff[" ..
                           SETTINGS.miniIconDist ..
                           "]|r - sets the minimap distance at which the gather icon will become iconic (0 = off, 1-60 = pixel radius on minimap, default = 40)");
    Gatherer_ChatPrint(
        "  |cffffffff/gather herbs (on|off|toggle|auto)|r |cff2040ff[" ..
            Gatherer_GetFilterVal("herbs") ..
            "]|r - select whether to show herb data on the minimap");
    Gatherer_ChatPrint(
        "  |cffffffff/gather mining (on|off|toggle|auto)|r |cff2040ff[" ..
            Gatherer_GetFilterVal("mining") ..
            "]|r - select whether to show mining data on the minimap");
    Gatherer_ChatPrint(
        "  |cffffffff/gather treasure (on|off|toggle|auto)|r |cff2040ff[" ..
            Gatherer_GetFilterVal("treasure") ..
            "]|r - select whether to show treasure data on the minimap");
    Gatherer_ChatPrint(
        "  |cffffffff/gather options|r - show/hide UI Options dialog.");
    ...
    Gatherer_ChatPrint(
        "  |cffffffff/gather loginfo (on|off)|r - show/hide logon information.");
    Gatherer_ChatPrint(
        "  |cffffffff/gather filterrec (herbs|mining|treasure)|r - link display filter to recording for selected gathering type");
    Gatherer_ChatPrint("  |cffffffff/gather debug ([on]|off)|r |cff2040ff[" ..
                           Gatherer_EBoolean[SETTINGS.debug] ..
                           "]|r - show/hide debug messages");
    Gatherer_ChatPrint("  |cffffffff/gather p2p ([on]|off)|r |cff2040ff[" ..
                           Gatherer_EBoolean[SETTINGS.p2p] ..
                           "]|r - enable/disable peer-to-peer functions");
  ...
  elseif (cmd == "filterrec") then
    ...
    if (value > -1) then
      if (SETTINGS.filterRecording[value]) then
        ...
        Gatherer_ChatPrint("Turned filter/recording link for " .. param ..
                               " off.");
      else
  ...
  elseif (cmd == "fperc") then
    ...
    Gatherer_ChatPrint("Setting fade percent at fade distance to " ..
                           SETTINGS.fadePerc);
    Gatherer_OnUpdate(0, true);
  ...
  elseif (cmd == "mainmap") then
    if ((param == "false") or (param == "off") or (param == "no") or
        (param == "0")) then
    ...
    if (SETTINGS.useMainmap and SETTINGS.showWorldMapFilters and
        SETTINGS.showWorldMapFilters == 1) then
  ...
  elseif ((cmd == "herbs") or (cmd == "mining") or (cmd == "treasure")) then
    ...
    elseif (param == "toggle" or param == "") then
      ...
      Gatherer_ChatPrint("Displaying " .. cmd ..
                             " notes in minimap based on ability");
    end
    Gatherer_OnUpdate(0, true);
    GatherMain_Draw();
  end
...
function Gatherer_OnEvent(event)
  ...
  elseif (event == "UI_ERROR_MESSAGE") then
    ...
    if (arg1 and
        (strfind(arg1, GATHERER_REQUIRE .. " " .. OLD_TRADE_HERBALISM) or
            strfind(arg1, GATHERER_NOSKILL .. " " .. OLD_TRADE_HERBALISM) or
            strfind(arg1, OLD_TRADE_HERBALISM .. " " .. GATHERER_NOSKILL) or
            strfind(arg1, GATHERER_REQUIRE .. " " .. TRADE_MINING) or
            strfind(arg1, GATHERER_NOSKILL .. " " .. TRADE_MINING) or
            strfind(arg1, TRADE_MINING .. " " .. GATHERER_NOSKILL))) then
  ...
  elseif (event == "SPELLCAST_START") then
    if (arg1 and
        (arg1 == GATHER_HERBALISM or arg1 == TRADE_MINING or arg1 ==
            TRADE_OPENING or arg1 == "")) then
  ...
  elseif (event == "WORLD_MAP_UPDATE") then
    if (WorldMapFrame:IsVisible()) then
      ...
      if (Gatherer_Settings.mapMinder == true and Gatherer_MapOpen == false) then
        ...
        if (Gatherer_CloseMap and
            (serverTime - Gatherer_CloseMap.time < Gatherer_Settings.minderTime)) then
  ...
  elseif (event == "PLAYER_LOGIN") then
    ...
    if (SETTINGS.logInfo and SETTINGS.logInfo == "on") then
      Gatherer_Print("[Player: " .. Gather_Player .. ", Theme: " ..
                         SETTINGS.iconSet .. ", Mainmap: " .. useMainmap ..
                         ", Minimap: " .. useMinimap .. ", MaxDist: " ..
                         SETTINGS.maxDist .. " units, NoteCount: " ..
                         SETTINGS.number .. ", Fade: " .. SETTINGS.fadePerc ..
                         "% at " .. SETTINGS.fadeDist .. " units, IconDist: " ..
                         SETTINGS.miniIconDist .. "px on minimap, MapMinder: " ..
                         mapMinder .. " (" .. minderTime .. "), Filters: herbs=" ..
                         Gatherer_GetFilterVal("herbs") .. ", mining=" ..
                         Gatherer_GetFilterVal("mining") .. ", treasure=" ..
                         Gatherer_GetFilterVal("treasure") .. "]");
    end
  ...
  elseif (event == "LEARNED_SPELL_IN_TAB" or event == "SPELLS_CHANGED") then
    ...
    if (GetNumSkillLines() > 0) then Gatherer_GetSkills(); end
...
function Gatherer_SetFilter(type, value)
  ...
  if (type) then Gatherer_Settings.filters[type] = value; end
end
...
function Gatherer_GetFilter(filter)
  ...
  elseif (value == "auto") then
    if (filter == "treasure") then filterVal = true; end
    if (not GatherSkills) then filterVal = true; end
...
function Gatherer_GetSkills()
  ...
  for i = 0, GetNumSkillLines(), 1 do
    local skillName, header, isExpanded, skillRank, _, _, _, _, _, _, _, _ =
        GetSkillLineInfo(i);

    -- expand the header if necessary
    if (header and not isExpanded) then GatherExpandedHeaders[i] = skillName; end
  end

  ExpandSkillHeader(0);
  for i = 1, GetNumSkillLines(), 1 do
    local skillName, header, _, skillRank, _, _, _, _, _, _, _, _ =
        GetSkillLineInfo(i);
    ...
    if (GatherSkills.herbs and GatherSkills.mining) then break end
  end

  -- close headers expanded during search process
  for i = 0, GetNumSkillLines() do
    local skillName, header, isExpanded, _, _, _, _, _, _, _, _, _ =
        GetSkillLineInfo(i);
...
local function random_choice(t)
  ...
  for i, o in pairs(t) do
    n = n + 1
    if math.random() < (1 / n) then choiceI, choiceO = i, o end
  end
...
local function selectRandomGather()
  ...
  return randomGather, randomNode.gtype, randomContinent, randomZone,
         randomNode.x, randomNode.y, randomNode.icon, eventType
end
...
function Gatherer_TimeCheck(timeDelta)
  ...
  if (Gatherer_UpdateTicker < 1) then return end
  ...
  if Gatherer_SecondsToAnnounce > 0 then return end
  -- the code below will run not more frequently
  ...
  if Gatherer_Settings.p2p then
    ...
    if Gatherer_Settings.debug then
      ...
      Gatherer_ChatNotify('Sending random node once in ' ..
                              Gatherer_AnnouncePeriod .. ' seconds.',
                          Gatherer_ENotificationType.sending);
      Gatherer_ChatNotify('args to send: ' .. table.concat(args, ', '),
                          Gatherer_ENotificationType.sending);
    end
...
function Gatherer_OnUpdate(timeDelta, force)
  ...
  if (Gatherer_InWorld == false) then return; end
  ...
  if (needsUpdate) then
    ...
    local xMovement = 0;
    if (ClosestGathers and ClosestGathers.px) then
      xMovement = math.abs(ClosestGathers.px - px);
    end
    local yMovement = 0;
    if (ClosestGathers and ClosestGathers.py) then
      yMovement = math.abs(ClosestGathers.py - py);
    end
    if (not ClosestGathers or ClosestGathers.playerC ~= continent or
        ClosestGathers.playerZ ~= zone or xMovement + yMovement > 0.01) then
      recalculate = true;
    end
    ...
    if (recalculate == true) then
      ...
      if (ClosestGathers.count > 0) then
        ...
        ClosestGathers.scaleX, ClosestGathers.scaleY =
            Gatherer_GetMapScale(continent, zone, inCity, zoomLevel);
      end
    else
      if ((inCity ~= ClosestGathers.inCity) or
          (zoomLevel ~= ClosestGathers.zoomLevel)) then
        ClosestGathers.inCity = inCity;
        ClosestGathers.zoomLevel = zoomLevel;
        ClosestGathers.scaleX, ClosestGathers.scaleY =
            Gatherer_GetMapScale(continent, zone, inCity, zoomLevel);
    ...
    if (ClosestGathers and ClosestGathers.count > 0) then
      ...
      for closestPos, closestGather in ClosestGathers.items do
        ...
        if (skip_node == 0) then
          ...
          local gatherNoteTexture = getglobal(
                                        "GatherNote" .. currentPos .. "Texture");

          local itemDeltaX = closestGather.deltax + playerDeltaX;
          local itemDeltaY = closestGather.deltay + playerDeltaY;
          local offsX, offsY, gDist = Gatherer_MiniMapPos(itemDeltaX,
                                                          itemDeltaY,
                                                          ClosestGathers.scaleX,
                                                          ClosestGathers.scaleY);
          ...
          local _, _, sDist = Gatherer_MiniMapPos(iDist / 10000, 0,
                                                  ClosestGathers.scaleX,
                                                  ClosestGathers.scaleY);
          ...
          if (SETTINGS.iconSet == "iconshade") then
            iconSet = "iconic";
            if (gDist < (math.floor(sDist) - 1)) then alpha = 0.4; end
          end
          if (not Gather_IconSet[iconSet]) then iconSet = "shaded"; end
          if (not Gather_IconSet[iconSet][textureType]) then
            textureType = "Default";
          end
          local selectedTexture =
              Gather_IconSet[iconSet][textureType][textureIcon];
          ...
          elseif ((not SETTINGS.NoIconOnMinDist or SETTINGS.NoIconOnMinDist == 0) and
              SETTINGS.alphaUnderMinIcon and gDist < (math.floor(sDist) - 1)) then
    ...
    while (maxPos < GATHERER_MAXNUMNOTES) do
      ...
      if (gatherNote:IsShown()) then gatherNote:Hide(); end
    end
  end
end
...
function Gatherer_CreateNoteObject(noteNumber)
  ...
  if (getglobal("GatherMain" .. noteNumber)) then
    ...
    button = CreateFrame("Button", "GatherMain" .. noteNumber, getglobal(
                             "GathererMapOverlayFrame" .. overlayFrameNumber),
                         "GatherMainTemplate");
...
function GatherMain_Draw()
  ...
  if ((Gatherer_MapOpen) and (SETTINGS.useMainmap)) then
    ...
    if ((mapContinent > 0) and (GatherItems[continentName]) and
      ...
      for gatherName, gatherData in GatherItems[continentName][zoneName] do
        ...
        if (not specificType) then specificType = "default"; end
        ...
        if (SETTINGS) then
          ...
          if (allowed == true and Gather_SkillLevel[specificType] and
              ...
              (minSetSkillLevel > Gather_SkillLevel[specificType] or
                  (SETTINGS.rareOre == 1 and
                      Gather_SkillLevel[Gather_RareMatch[specificType]] and
                      Gather_SkillLevel[specificType] <
                      Gather_SkillLevel[Gather_RareMatch[specificType]] and
                      minSetSkillLevel >
                      Gather_SkillLevel[Gather_RareMatch[specificType]]))) then
            allowed = false;
          end
        end

        if ((allowed == true) and
            ((SETTINGS == nil) or (SETTINGS.interested == nil) or
                (idx_count == 0) or SETTINGS.interested[gatherType] == nil or
                (SETTINGS.interested[gatherType][specificType] == true or
                    (SETTINGS.rareOre == 1 and
                        SETTINGS.interested[gatherType][Gather_RareMatch[specificType]])))) then
          for hPos, gatherInfo in gatherData do
            local convertedGatherType = "";
            local numGatherType, numGatherIcon;

            if (lastUnused > 1000 and lastUnused < maxNotes and
                mod(lastUnused, 100) == 0) then
              local overlayFrameNumber = math.floor(
                                             (lastUnused - 1000) / 100 + 1);
            ...
            if ((gatherInfo.x) and (gatherInfo.y) and (gatherInfo.x > 0) and
                (gatherInfo.y > 0) and (lastUnused <= maxNotes)) then
              ...
              mainNote:SetPoint("CENTER", "GathererMapOverlayFrame", "TOPLEFT",
                                mnX, mnY);

              if (SETTINGS and SETTINGS.ToggleWorldNotes and
                  SETTINGS.ToggleWorldNotes == 1) then
              ...
              if (type(specificType) == "number") then
                specificType = Gatherer_GetDB_IconIndex(specificType,
                                                        convertedGatherType);
              end
              local texture =
                  Gather_IconSet["iconic"][convertedGatherType][specificType];
              if (not texture) then
              ...
              local mainNoteTexture = getglobal(
                                          "GatherMain" .. lastUnused ..
                                              "Texture");
              ...
              if (not mainNote:IsShown()) then mainNote:Show(); end
  ...
  for i = lastUnused, maxNotes, 1 do
    ...
    if (i < maxNotes and mod(i, 100) == 0) then
    ...
    if (mainNote and mainNote:IsShown()) then
      mainNote:SetPoint("CENTER", "GathererMapOverlayFrame", "TOPLEFT",
                        Gatherer_WorldMapDetailFrameWidth + 16,
                        Gatherer_WorldMapDetailFrameHeight + 16);
    end
  end
...
function Gatherer_FindClosest(num, interested)
  ...
  for gatherName, gatherData in GatherItems[continentName][zoneName] do
    ...
    if ((allowed == true) and
        ((SETTINGS == nil) or (SETTINGS.interested == nil) or (idx_count == 0) or
            SETTINGS.interested[gatherType] == nil or
            (SETTINGS.interested[gatherType][specificType] == true or
                (SETTINGS.rareOre == 1 and
                    SETTINGS.interested[gatherType][Gather_RareMatch[specificType]])))) then
      local maxNum = 100;
      ...
      for _, gatherInfo in gatherData do
        ...
        absHx, absHy = Gatherer_AbsCoord(continent, zone, gatherInfo.x / 100,
                                         gatherInfo.y / 100);
        ...
        if ((absHx ~= 0) and (absHy ~= 0)) then
          ...
          local dist, deltaX, deltaY = Gatherer_Distance(absPx, absPy, absHx,
                                                         absHy);
          ...
          if ((maxAllowable == 0) or (dist < maxAllowable)) then
            ...
            for localPos, localInfo in gatherLocal.items do
              ...
              if (localPos <= num) then
                if (localPos > maxLocalPos) then
                  maxLocalPos = localPos;
                end

                -- take node at the same approximate spot into consideration
                if ((abs(gatherInfo.x - localInfo.item.x) <=
                    GATHERER_CLOSESTCHECK or Gatherer_Round(gatherInfo.x * 10) ==
                    Gatherer_Round(localInfo.item.x * 10)) and
                    (abs(gatherInfo.y - localInfo.item.y) <=
                        GATHERER_CLOSESTCHECK or
                        Gatherer_Round(gatherInfo.y * 10) ==
                        Gatherer_Round(localInfo.item.y * 10))) then
                  replCandidate = maxNum;
                  maxNum = maxNum + 1;
                  -- shorter distance
                elseif (localInfo.dist > dist) then
                  if ((replCandidate == 0) or
                      (gatherLocal.items[replCandidate] and
                          (localInfo.dist >
                              gatherLocal.items[replCandidate].dist))) then
            ...
            elseif (maxLocalPos == num and gatherLocal.items[replCandidate] and
                dist > gatherLocal.items[replCandidate].dist) then
              replCandidate = 0;
            end
...
function Gatherer_GetMapScale(continent, zone, inCity, zoomLevel)
  ...
  if ((not GatherRegionData) or (not GatherRegionData[continent]) or
...
function Gatherer_ReadBuff(event, fishItem, fishTooltip)
  ...
  elseif (strfind(event, "CHAT_MSG") or
      strfind(event, "CHAT_MSG_SPELL_SELF_BUFF")) then
    if (string.find(chatline, HERB_GATHER_STRING)) then
      record = true;
      gather = string.lower(
                   strsub(chatline, HERB_GATHER_LENGTH, HERB_GATHER_END))
      gatherType = 1;
      gatherIcon = gather;
    ...
    elseif (string.find(chatline, TREASURE_GATHER_STRING)) then
      record = true;
      gather = string.lower(strsub(chatline, TREASURE_GATHER_LENGTH,
                                   TREASURE_GATHER_END));
  ...
  elseif (strfind(event, "UI_ERROR_MESSAGE") or
      (strfind(event, "SPELLCAST_STOP") and Gatherer_RecordFlag == 1)) then
    -- process event to record, normally not possible gather (low/inexistant skill)
    gather = string.lower(Gatherer_ExtractItemFromTooltip());
    if (Gatherer_currentNode and Gatherer_currentNode ~= gather and event ==
        "SPELLCAST_STOP" and Gatherer_RecordFlag == 1) then
      gather = string.lower(Gatherer_currentNode);
    end
    if (not arg1 and Gatherer_currentAction == nil and event == "SPELLCAST_STOP" and
        Gatherer_RecordFlag == 1) then
      chatline = "";
    elseif (event ~= "UI_ERROR_MESSAGE") then
      chatline = Gatherer_currentAction;
    end
    -- process non gatherable item because of low/lack of skill
    if (gather and not gather ~= "" and
        (strfind(chatline, TRADE_HERBALISM) or
            strfind(chatline, OLD_TRADE_HERBALISM) or
            strfind(chatline, GATHER_HERBALISM))) then -- Herb
    ...
    elseif (gather and not gather ~= "" and strfind(chatline, TRADE_MINING)) then -- Ore
      ...
      gatherIcon = Gatherer_FindOreType(gather);
      gatherEventType = 1;
    elseif (gather and not gather ~= "" and
        (strfind(chatline, TRADE_OPENING) or chatline == "")) then -- Treasure
...
local function insertionGatherIndex(gatherList, maxCheckDist, gatherX, gatherY)
  ...
  for gatherIndex, gatherData in gatherList do
    ...
    local dist, deltaX, deltaY = Gatherer_Distance(gatherX, gatherY,
                                                   gatherData.x, gatherData.y);
    ...
    if (gatherIndex > lastGather) then lastGather = gatherIndex; end
  end
  ...
  if (newNodeFound) then
    ...
    if thereIsNumerationGap then resultIndex = firstNumerationGap; end
  end
  return resultIndex, newNodeFound, gatherX, gatherY
end

if GatherItems == nil then GatherItems = {} end
function Gatherer_AddGatherToBase(gather, gatherType, gatherC, gatherZ, gatherX,
                                  gatherY, iconIndex, gatherEventType,
                                  updateCount)
...
function Gatherer_AddGatherHere(gather, gatherType, gatherIcon, gatherEventType)
  ...
  if (Gatherer_Settings.filterRecording[gatherType] and
      not Gatherer_Settings.interested[gatherType][gatherIcon]) then return; end
  ...
  if Gatherer_Settings.p2p then
    -- Broadcast to guild
    Gatherer_BroadcastGather(gather, gatherType, gatherContinent, gatherZone,
                             gatherX, gatherY, iconIndex, gatherEventType)
  end

  Gatherer_AddGatherToBase(gather, gatherType, gatherContinent, gatherZone,
                           gatherX, gatherY, iconIndex, gatherEventType, true);
  Gatherer_OnUpdate(0, true);
  GatherMain_Draw();
end
...
function Gatherer_Show()
  local gatherCont, gatherZone, gatherName, contData, zoneData, nameData,
        gatherPos, gatherItem;
    ...
    for gatherZone, zoneData in contData do
      for gatherName, nameData in zoneData do
        for gatherPos, gatherItem in nameData do
          Gatherer_Print(Gatherer_EGatherType[gatherItem.gtype] .. " " ..
                             gatherName .. " was found in zone " .. gatherCont ..
                             ":" .. gatherZone .. " at " .. gatherItem.x .. "," ..
                             gatherItem.y .. "  (" .. gatherItem.count ..
                             " times)");
        end
      end
    end
...
function Gatherer_Print(str, add)
  if ((Gather_LastPrinted) and (str == Gather_LastPrinted)) then return; end
  Gather_LastPrinted = str;
  if (add) then str = str .. ": " .. add; end
  -- adds a message into the combat log
  if (ChatFrame2) then ChatFrame2:AddMessage(str, 1.0, 1.0, 0.0); end
end

function Gatherer_Debug(str, add)
  if not (type(Gatherer_DebugFrame) == "table" and
      Gatherer_DebugFrame.AddMessage) then return; end
  if (add) then str = str .. ": " .. add; end
...
function Gatherer_MakeName(frameID)
  ...
  if (GATHERER_LOADED) then
    ...
    for id in tmpClosest.items do
      if (id ~= frameID and
          (abs(gatherInfo.item.x - tmpClosest.items[id].item.x) <=
              GATHERER_CLOSESTCHECK or Gatherer_Round(gatherInfo.item.x * 10) ==
              Gatherer_Round(tmpClosest.items[id].item.x * 10)) and
          (abs(gatherInfo.item.y - tmpClosest.items[id].item.y) <=
              GATHERER_CLOSESTCHECK or Gatherer_Round(gatherInfo.item.y * 10) ==
              Gatherer_Round(tmpClosest.items[id].item.y * 10))) then
        tmpItemIDtable[tmpCount] = {};
        tmpItemIDtable[tmpCount].name = Gatherer_GetMenuName(
                                            tmpClosest.items[id].name);
        tmpItemIDtable[tmpCount].count = tmpClosest.items[id].item.count;
        tmpItemIDtable[tmpCount].dist = math.floor(
                                            tmpClosest.items[id].dist * 10000) /
                                            10;
...
function Gatherer_ChangeMap()
  ...
  if ((Gatherer_CloseMap ~= nil) and
      (Gatherer_CloseMap.continent == Gatherer_LastZone.continent) and
      (Gatherer_CloseMap.zone == Gatherer_LastZone.zone)) then
    minderCurZone = true;
  end
  ...
  if ((playerContinent == mapContinent) and (playerZone == mapZone)) then
    return true;
  end
...
function Gatherer_PlayerPos(forced)
  ...
  if (currentZoneName ~= Gather_LastZone) then forced = true; end
...
function Gatherer_ModifyItem()
  ...
  if (GatherMainMapItem.newIcon and GatherMainMapItem.newIcon ~=
      GatherMainMapItem.gatherIcon) then
    newIcon = GatherMainMapItem.newIcon;
  else
    newIcon = GatherMainMapItem.gatherIcon;
  end

  if (GatherMainMapItem.newType and GatherMainMapItem.newType ~=
      GatherMainMapItem.gatherType) then
  ...
  if (GatherMainMapItem.scope == "Node") then
    ...
    if (GatherMainMapItem.newGatherName and mod_name ~=
      ...
      if (not GatherItems[mod_continent][mod_zone][newGatherName]) then
        ...
        GatherItems[mod_continent][mod_zone][newGatherName][GatherMainMapItem.localIndex] =
  ...
  elseif (GatherMainMapItem.scope == "Zone") then
    ...
    if (GatherMainMapItem.newGatherName and mod_name ~=
      ...
      GatherItems[mod_continent][mod_zone][newGatherName] = {};
      GatherItems[mod_continent][mod_zone][newGatherName] =
          GatherItems[mod_continent][mod_zone][mod_name];
    ...
    if (newIcon ~= GatherMainMapItem.gatherIcon) then
      ...
      for index in GatherItems[mod_continent][mod_zone][newGatherName] do
        GatherItems[mod_continent][mod_zone][newGatherName][index].icon =
            newIcon;
      end
    end
    ...
    if (newType ~= GatherMainMapItem.gatherType) then
      local index;
      for index in GatherItems[mod_continent][mod_zone][newGatherName] do
        GatherItems[mod_continent][mod_zone][newGatherName][index].gtype =
            newType;
      end
    end
    ...
    for search_index in GatherItems[mod_continent] do
      if (GatherItems[mod_continent][search_index][mod_name]) then
        ...
        if (GatherMainMapItem.newGatherName and mod_name ~=
          ...
          GatherItems[mod_continent][search_index][newGatherName] =
              GatherItems[mod_continent][search_index][mod_name];
        ...
        if (newIcon ~= GatherMainMapItem.gatherIcon) then
          ...
          for index in GatherItems[mod_continent][search_index][newGatherName] do
            GatherItems[mod_continent][search_index][newGatherName][index].icon =
                newIcon;
          end
        end
        ...
        if (newType ~= GatherMainMapItem.gatherType) then
          local index;
          for index in GatherItems[mod_continent][search_index][newGatherName] do
            GatherItems[mod_continent][search_index][newGatherName][index].gtype =
                newType;
          end
        end
      end
...
function Gatherer_WMDropDownGType_Initialize()
  ...
  if (GatherMainMapItem.newType) then iconGtype = GatherMainMapItem.newType; end
  ...
  if (type(GatherMainMapItem.gatherType) == "number") then
    UIDropDownMenu_SetText(Gatherer_EGatherType[GatherMainMapItem.gatherType],
                           Gatherer_WMDropDownGType);
  else
    UIDropDownMenu_SetText(GatherMainMapItem.gatherType,
                           Gatherer_WMDropDownGType);
  end
end
...
function Gatherer_WMDropDownGType_OnClick()
  ...
  GatherMainMapItem.newType = Gatherer_EGatherType[UIDropDownMenu_GetText(
                                  Gatherer_WMDropDownGType)];

  UIDropDownMenu_ClearAll(Gatherer_WMDropDownIcons);
  UIDropDownMenu_Initialize(Gatherer_WMDropDownIcons,
                            Gatherer_WMDropDownIcons_Initialize);
end
...
function Gatherer_WMDropDownIcons_Initialize()
  ...
  UIDropDownMenu_SetText(Gatherer_GetDB_IconIndex(GatherMainMapItem.gatherIcon,
                                                  iconGtype),
                         Gatherer_WMDropDownIcons);
end

function Gatherer_WMDropDownIcons_OnClick()
  UIDropDownMenu_SetSelectedID(Gatherer_WMDropDownIcons, this:GetID());
  GatherMainMapItem.newIcon = Gatherer_GetDB_IconIndex(
                                  UIDropDownMenu_GetText(
                                      Gatherer_WMDropDownIcons),
                                  GatherMainMapItem.gatherType);
end
